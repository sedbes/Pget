...
class Pget
{
    ...
    */
    ...
    private function createHandle($url)
    {
        ...
        curl_setopt($ch, CURLOPT_REFERER, $this->cfg['start_info']['domain'] ?? $url);
    ...
    private function fillMap(): void
    {
        // 超过最大重试次数就停止
        if ($this->error_count > $this->cfg['--tries']) {
            $this->echo_logs("ERROR: Too many retries");
            $this->flush_log_buffer();
        ...
        while (count($this->handleMap) < $this->cfg['--max-threads'] && $this->pending_queue->isEmpty() === false) {
            ...
            // 输出请求日志信息
            $this->echo_logs($number, $url, date('Y-m-d H:i:s'), 'Task Add');
            ...
            // 判断是否需要重新下载
            if ($is_file_exist && $this->cfg['--no-clobber']) {
                $this->echo_logs('FORCEECHO', $number, date('Y-m-d H:i:s'), "{$url} -> {$local_file}", 'Exists, Skip');
                // 跳过本地已存在的URL
                continue;
            }
            // 不论后面结果如何，先把这些URL也设置为true，表示已处理
            if (isset($url2)) $this->link_table_add_item($url2, true);
            if (isset($url3)) $this->link_table_add_item($url3, true);
            // ==========下载前处理过程：结束=========
            ...
            $url_getting = rawurlencodex($url);
            // 创建子句柄
            $ch = $this->createHandle($url_getting);
            //加到总句柄中
            curl_multi_add_handle($this->chs, $ch);
            //记录到正在处理的句柄中
            $this->handleMap[$ch] = ['url' => $url, 'local_file' => $local_file, 'is_file_exist' => $is_file_exist, 'id' => $number];
        }
    }
    ...
    public function sun_singlecatcher()
    {
        try {
            // 当最大线程数≤1时，退化为串行模式
    ...
    public function success($ch, $response, $http_info): void
    {
        ...
        $url = $this->handleMap[$ch]['url'];
        // 本地文件
        $local_file = $this->handleMap[$ch]['local_file'];
        // 是否已存在
        $is_file_exist = $this->handleMap[$ch]['is_file_exist'];
        // 无论响应是否空，标记存在后可，跳过反复下载该URL
        $this->link_table_add_item($url, true);
        ...
        if ($this->cfg['--adjust-extension']) {
            ...
            $has_ext = false;
            foreach ($this->extensions as $i) {
                if (str_ends_with($this->handleMap[$ch]['local_file'], $i)) {
                    $has_ext = true;
                    break;
                }
            }
        ...
        if (!isset($dir_cache[$local_dir])) {
            if (!is_dir($local_dir) && !mkdir($local_dir, 0777, true)) {
                $this->echo_logs('Failed to create directory: ' . $local_dir);
            }
            $dir_cache[$local_dir] = true;
        }
        // 判断是否需要保存文件。上方有 --adjust-extension 添加扩展名导致 is_file_exist 被修正的情况，故需要再次判断
        if ($is_file_exist && $this->cfg['--no-clobber']) {
            $this->echo_logs($number, date('Y-m-d H:i:s'), "{$url} -> {$local_file}", 'Unwrite');
            return;
        } elseif (@file_put_contents($local_file, $response) === false) {
            $this->echo_logs('FORCEECHO', $number, date('Y-m-d H:i:s'), $url, 'Failed to write file: ' . $local_file);
        }
